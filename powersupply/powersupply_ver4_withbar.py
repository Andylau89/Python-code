...
# 第3步，设定窗口的大小(长 * 宽)
window.geometry('1600x950')  # 这里的乘是小x
...
channel_1.place(x=20, y=350, width=channel_frame_width)
...
# 第6步，定义一个触发函数功能
def vol_set_2(v):
    channel_2_V_read.config(text='Channel_2_Voltage: ' + v)
    output_cmd = 'VSET2 ' + v
    m01.send(output_cmd)

def cur_set_2(v):
    channel_2_I_read.config(text='Channel_2_Current: ' + v)
    output_cmd = 'ISET2 ' + v
    # print(output_cmd)
    m01.send(output_cmd)
...
def cur_set_1(v):
    ...
    m01.send(output_cmd)
######################################################################################################################
channel_1_bar = tk.LabelFrame(window, text="Channel_1_bar", padx=10, pady=10)
channel_1_bar.place(x=800, y=30, width=360)

channel_1_V_read = tk.Label(channel_1_bar, bg='green', fg='white', width=20, text='Channel_1_Voltage')
channel_1_V_read.grid(column=0,row=0)

channel_1_I_read = tk.Label(channel_1_bar, bg='green', fg='white', width=20, text='Channel_1_Current')
channel_1_I_read.grid(column=1,row=0)



# 第5步，创建一个尺度滑条，长度200字符，从0开始10结束，以2为刻度，精度为0.01，触发调用print_selection函数
tk.Scale(channel_1_bar, label='Voltage_Set',troughcolor='gray',bg='green', from_=5, to=0, orient=tk.VERTICAL, length=800, showvalue=1, tickinterval=2,
             resolution=0.01, command=vol_set_1).grid(column=0, row=1)

tk.Scale(channel_1_bar, label='Current_Set', troughcolor='gray',bg='yellow',from_=4, to=0, orient=tk.VERTICAL, length=800, showvalue=1, tickinterval=2,
             resolution=0.001, command=cur_set_1).grid(column=1, row=1)

######################################################################################################################

channel_2_bar = tk.LabelFrame(window, text="Channel_2_bar", padx=10, pady=10)
channel_2_bar.place(x=1200, y=30, width=360)

channel_2_V_read = tk.Label(channel_2_bar, bg='green', fg='white', width=20, text='Channel_1_Voltage')
channel_2_V_read.grid(column=0,row=0)

channel_2_I_read = tk.Label(channel_2_bar, bg='green', fg='white', width=20, text='Channel_1_Current')
channel_2_I_read.grid(column=1,row=0)



#创建一个尺度滑条，长度200字符，从0开始10结束，以2为刻度，精度为0.01，触发调用print_selection函数
tk.Scale(channel_2_bar, label='Voltage_Set',troughcolor='gray',bg='green', from_=5, to=0, orient=tk.VERTICAL, length=800, showvalue=1, tickinterval=2,
             resolution=0.01, command=vol_set_2).grid(column=0, row=1)

tk.Scale(channel_2_bar, label='Current_Set', troughcolor='gray',bg='yellow',from_=4, to=0, orient=tk.VERTICAL, length=800, showvalue=1, tickinterval=2,
